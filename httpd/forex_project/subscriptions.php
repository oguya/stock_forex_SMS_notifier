...
function fetchSMS(){
	...
	$sql="select requestID,MSISDN,messageContent,processed from incomingRequests where processed = 5 limit 10";
	$result=mysql_query($query);
	if(!$result){
		$dbErr="SQL Error: ".mysql_error()."SQL CODE: ".$sql;
		flog($dbErr, $_SERVER["SCRIPT_FILENAME"]);
	}else{
		$subLogs = "Fetched 1 sms bucket of approx 10 items.";
...
function updateSMS($requestID){
	...
	for ($x=0;$x<(count($requestID)); $x++){
		$sql_update = "update incomingRequests set status = $status where requestID = $requestID[$x] limit 1";
...
function invalidKeyword($destAddr,$keyword){
	...
	$sql="insert into outbound(destAddr,sourceAddr,messageType,messageContent,status ) 
	values('$destAddr',default,default,'$textErr','send')";
...
function subscriptionStatus($destAddr,$keyword,$state){
	if($state == 'unsubscribe'){
		...
		$sql_unsub="insert into outbound(destAddr,sourceAddr,messageType,messageContent,status) 
		values('$destAddr',default,default,'$response','send')";
		...
		$sql_sub = "insert into outbound(destAddr,sourceAddr,messageType,messageContent,status) 
		values('$destAddr',default,default,'$response','send')";
...
function Newsubscriptions($command,$subscriber,$keyword){
	...
	$sql_newSub = "insert into subscriptions(channelID,msisdn,dateSubscribed) 
	values($channelID,'$subscriber',now())";
...
function delSubs($command,$subscriber,$keyword){
	$sql_del="update subscriptions set active = 0 where msisdn like '%$subscriber%' limit 1";
...
function currencyAlerts(){
	$currencies = array();
	$sql="select serviceName,services.currencyID,currencies.fullName,currencies.abbreviation 
	from services inner join currencies on currencies.currencyID=services.currencyID";
	$result=mysql_query($sql);
	if(!$result){
		$dbErr="SQL Error: ".mysql_error()."SQL CODE: ".$sql_alerts;
		flog($dbErr, $_SERVER["SCRIPT_FILENAME"]);
	}else{
		$subsLogs = "Successfully fetched market rates for KES".$sql_alerts;
		logSubs($subsLogs, $_SERVER["SCRIPT_FILENAME"]);
...
function genAlerts($currency){
	...
	## for KES
	$sql_alerts = "select currencyID,market,rate,dateCreated from Rates where 
	market in ('$currency/USD','$currency/AUD','$currency/CAD','$currency/CHF','$currency/EUR','$currency/GBP','$currency/JPY','$currency/NZD','$currency/ZAR') and 
	date(dateCreated)=curdate() and substr(timediff(time(now()),time(dateCreated)),1,2) < '02' ";
	...
	$channelID=1;
	$sql_newAlerts = "insert into alerts(channelID,sourceAddr,messageContent,messageStatus) 
	values($channelID,default,'$alerts','send')";
...
function sendAlerts(){
	$sql="select  alertID,alerts.channelID,msisdn,messageContent,messageStatus from alerts 
	inner join subscriptions on subscriptions.channelID = alerts.channelID and messageStatus= 'send'";
	$result = mysql_query($sql);
	//logging
	if(!$result){
		$dbErr = "SQL Error: ".mysql_error()."SQL CODE: ".$sql;
	...
	for($x=0;$x<(count($alertID)); $x++){
		$sql_send="insert into outbound(destAddr,sourceAddr,messageType,messageContent,status)  
		values('$destAddr[$x]',default,default,'$messageContent[$x]','send')";
